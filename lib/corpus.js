...
function enrichPassage(passage) {
  ...
  return {
    ...
    period: passage.period ?? work?.period ?? "late",
    publicationYear: passage.publicationYear ?? work?.publicationYear ?? "",
    writtenYear: passage.writtenYear ?? work?.writtenYear ?? "",
    dateNote: passage.dateNote ?? work?.dateNote ?? "",
    readingUrl: passage.readingUrl ?? work?.readingUrl ?? "",
  };
}
